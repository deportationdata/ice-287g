...
non_facility_sf <- st_read("data/intermediate/match-non-facility.parquet", quiet = TRUE)
...
non_facility_ids <- non_facility_sf |>
  st_drop_geometry() |>
  filter(coalesce(!match_type %in% c("regional_member_municipality", "judicial_district_member_county"), TRUE)) |>
  pull(agreement_id)
stopifnot(
  "every agreement is in exactly one layer" =
    setequal(c(non_facility_sf$agreement_id, facility_sf$agreement_id), agreements$agreement_id) &&
      !any(facility_sf$agreement_id %in% non_facility_sf$agreement_id),
  "only regional departments and judicial districts fan out in the non-facility layers" = !anyDuplicated(non_facility_ids),
  "the facility layer holds exactly the facility-point agreements" =
    setequal(facility_sf$agreement_id, agreements$agreement_id[agreements$geom_class == "facility_point"])
)
...
compose_review_reason <- function(flags) {
  stopifnot("every review flag is in the vocabulary" = setequal(names(flags), review_vocabulary$flag))
...
all_agreements_sf <-
  ...
  mutate(
    across(c(leaic_fips_mismatch, lear_fips_mismatch, crime_fips_mismatch, hifld_fips_mismatch),
  ...
  mutate(
    ...
    ambiguous_candidates = coalesce(ambiguous_candidates, FALSE) & !leaic_place_confirmed,
    type_mismatch = coalesce(type_mismatch, FALSE) & !leaic_place_confirmed,
    roster_county_disagrees = coalesce(leaic_fips_mismatch, FALSE) | coalesce(lear_fips_mismatch, FALSE) |
      coalesce(crime_fips_mismatch, FALSE) | coalesce(hifld_fips_mismatch, FALSE),
    across(c(geometry_unmatched, fuzzy_match, weak_match, university_address_mismatch,
             university_county_mismatch, ori_conflict, ori_ambiguous), \(x) coalesce(x, FALSE)),
    moa_pending = coalesce(moa == "pending", FALSE),
    has_addendum = !is.na(addendum)
  )
all_agreements_sf <- all_agreements_sf |>
  mutate(
    review_reason = compose_review_reason(st_drop_geometry(all_agreements_sf)[review_vocabulary$flag]),
    needs_review = !is.na(review_reason),
    match_quality = case_when(
      geometry_unmatched ~ "unmatched",
      str_detect(coalesce(match_type, ""), "^manual") | coalesce(match_type %in% c("regional_member_municipality", "judicial_district_member_county"), FALSE) ~ "manual",
      fuzzy_match ~ "fuzzy",
      weak_match ~ "weak",
...
sf::st_write(all_agreements_sf, dsn = "data/intermediate/match-all-features.parquet", driver = "Parquet", layer_options = "USE_PARQUET_GEO_TYPES=ONLY", delete_dsn = file.exists("data/intermediate/match-all-features.parquet"), quiet = TRUE)
...
quality_order <- c("unmatched", "weak", "fuzzy", "exact_flagged", "manual", "exact")
...
agreement_level_sf <- agreement_level_sf |>
  mutate(
    review_reason = compose_review_reason(st_drop_geometry(agreement_level_sf)[review_vocabulary$flag]),
    needs_review = !is.na(review_reason),
    has_addendum = !is.na(addendum),
    moa_pending = coalesce(moa == "pending", FALSE)
  ) |>
...
stopifnot(
  "agreement-level dataset must hold every agreement exactly once" =
    setequal(agreement_level_sf$agreement_id, agreements$agreement_id) &&
      !anyDuplicated(agreement_level_sf$agreement_id)
)

sf::st_write(agreement_level_sf, dsn = "data/agreement-level-sf.parquet", driver = "Parquet", layer_options = "USE_PARQUET_GEO_TYPES=ONLY", delete_dsn = file.exists("data/agreement-level-sf.parquet"), quiet = TRUE)
