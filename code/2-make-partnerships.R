# One record per partnership across every era. ICE's own record comes from the
...
# partnerships by rule, and arbitrated against ICE with the winning source beside
# each value; where a source and ICE differ the difference is recorded, never averaged
# -> data/partnerships.parquet, data/intermediate/historical-source-claims.csv,
#    data/intermediate/historical-source-claims-unresolved.csv, data/partnership-disagreements.csv,
...
ice <- arrow::read_parquet("data/intermediate/identity-partnerships.parquet")
...
# only an agreement ICE announced in a press release may add a partnership; every
# other source attests to partnerships ICE's sheets already carry
...
# --- resolve every claim to a partnership ---------------------------------------
registry <- partnership_registry(ice, spellings, aliases)
...
resolved <- resolve_partnership(claims$state, claims$agency_raw, claims$source_id %in% MINTING_SOURCES,
                                registry, ice, identities, xwalk, aliases, signed = row_signed)
claims <- claims |>
  mutate(partnership_id = resolved$partnership_id, resolution = resolved$resolution,
...
stopifnot(
  ...
  "a resolved claim carries a partnership id" = all(is.na(claims$partnership_id) == str_starts(claims$resolution, "unresolved")),
  "only a minting source may mint" = all(claims$resolution != "minted" | claims$source_id %in% MINTING_SOURCES)
)
pa <- claims |> filter(!is.na(partnership_id))

# --- the universe: ICE's partnerships plus what a minting source added ----------
minted <- pa |>
  filter(resolution == "minted") |>
  arrange(as_of) |>
  distinct(partnership_id, .keep_all = TRUE) |>
  transmute(partnership_id, state, state_key = norm_state(state), display_agency = agency_raw) |>
...
signed_claims <- bind_rows(
  identities |> transmute(partnership_id, source_id = "ice_sheet", value_date = signed),
  pa |> filter(field == "signed", !is.na(value_date)) |> select(partnership_id, source_id, value_date)
)
first_signed <- signed_claims |>
  arrange(partnership_id, value_date, source_id != "ice_sheet", source_id) |>
  distinct(partnership_id, .keep_all = TRUE) |>
  transmute(partnership_id, first_signed = value_date, first_signed_source = source_id)
signing_dates <- signed_claims |>
  distinct(partnership_id, value_date, source_id) |>
  arrange(partnership_id, value_date, source_id) |>
  summarise(sources = paste(source_id, collapse = ","), .by = c(partnership_id, value_date)) |>
  summarise(signing_dates = paste(sprintf("%s (%s)", value_date, sources), collapse = "; "),
            latest_signed = max(value_date), .by = partnership_id)
...
attested <- pa |>
  ...
  summarise(attested_active_from = min(spoken_to), attested_active_to = max(spoken_to),
            source_ids = paste(sort(unique(source_id)), collapse = "; "), .by = partnership_id)
rescinded <- pa |>
  filter(field == "rescinded", !is.na(value_date)) |>
  arrange(partnership_id, desc(value_date)) |>
  distinct(partnership_id, .keep_all = TRUE) |>
  transmute(partnership_id, rescinded_on = value_date, rescinded_source = source_id)

moa <- bind_rows(
  pa |> filter(field == "moa_file", !is.na(value)) |> transmute(partnership_id, moa_file = value),
  identities |> filter(str_detect(coalesce(raw_moa_last, ""), "^https?://")) |>
    transmute(partnership_id, moa_file = basename(raw_moa_last)),
  agreements |> filter(str_detect(coalesce(moa, ""), "^https?://")) |> transmute(partnership_id, moa_file = basename(moa))
) |>
  distinct(partnership_id, moa_file)
...
# The partnership takes its active or latest agreement's level and county; a partnership ICE
...
level_modern <- agreements |>
  ...
  transmute(partnership_id, jurisdiction_level, county_fips)

# --- assemble ---------------------------------------------------------------------
partnerships <- bind_rows(ice |> select(partnership_id, state, state_abbr, state_key, canonical_agency, display_agency) |>
                            mutate(ice_published = TRUE),
                          minted |> mutate(ice_published = FALSE)) |>
  arrange(partnership_id, desc(ice_published)) |>
  distinct(partnership_id, .keep_all = TRUE) |>
  left_join(ice_record, by = "partnership_id") |>
  left_join(first_signed, by = "partnership_id") |>
  left_join(signing_dates, by = "partnership_id") |>
  left_join(attested, by = "partnership_id") |>
  left_join(rescinded, by = "partnership_id") |>
  left_join(moa_summary, by = "partnership_id") |>
  left_join(level_modern, by = "partnership_id") |>
  mutate(
  ...
  select(partnership_id, state, state_abbr, display_agency, jurisdiction_level, county_fips, ice_published, is_current,
...
stopifnot("every jurisdiction level is one of the eight" =
            all(is.na(partnerships$jurisdiction_level) | partnerships$jurisdiction_level %in% JURISDICTION_LEVELS))
if (anyNA(partnerships$jurisdiction_level)) {
  warning(sum(is.na(partnerships$jurisdiction_level)), " partnership(s) have no jurisdiction level: ",
          paste(partnerships$partnership_id[is.na(partnerships$jurisdiction_level)], collapse = ", "))
}

# --- disagreements: every place a source and ICE differ -------------------------
p_state <- partnerships |> select(partnership_id, p_state = state)
dis_state <- pa |>
  inner_join(p_state, by = "partnership_id") |>
  filter(!is.na(state), state != p_state) |>
  distinct(partnership_id, source_id, state, p_state) |>
  transmute(partnership_id, kind = "state", field = "state", value_a = state, source_a = source_id,
...
dis_date <- pa |>
  ...
  inner_join(identities |> summarise(ice_dates = list(unique(signed)), .by = partnership_id), by = "partnership_id") |>
  ...
  distinct(partnership_id, source_id, value_date, nearest) |>
  transmute(partnership_id, kind = "date", field = "signed", value_a = as.character(value_date), source_a = source_id,
...
dis_model <- pa |>
  ...
  inner_join(identities |> summarise(ice_models = list(unique(support_abbr(support_key))), .by = partnership_id),
             by = "partnership_id") |>
  filter(!map2_lgl(value, ice_models, \(m, s) model_map[[m]] %in% s)) |>
  distinct(partnership_id, source_id, value, ice_models) |>
  transmute(partnership_id, kind = "model", field = "model", value_a = value, source_a = source_id,
...
# is each partnership there or not, on both sides
pub_members <- obs_ids |>
  filter(!is.na(partnership_id)) |>
  distinct(publication_id, partnership_id) |>
...
dis_presence <- pa |>
  ...
  pmap(\(source_id, as_of) {
    ...
    ice_set <- pub_members |> filter(publication_id == nearest$publication_id) |> pull(partnership_id)
    src_set <- pa |> filter(source_id == !!source_id, as_of == !!as_of, field == "listed") |> pull(partnership_id) |> unique()
    bind_rows(
      tibble(partnership_id = setdiff(src_set, ice_set), value_a = "listed", source_a = source_id,
             value_b = "absent", source_b = nearest$publication_id),
      tibble(partnership_id = setdiff(ice_set, src_set), value_a = "absent", source_a = source_id,
...
dis_ambiguous <- claims |>
  ...
  transmute(partnership_id = NA_character_, kind = "ambiguous_resolution", field = "name", value_a = agency_raw,
            source_a = source_id, value_b = state, source_b = "registry", published_value = NA_character_,
            resolution_rule = "the name begins more than one partnership in the state; left unresolved")
# one MOA file linked for two partnerships: ICE's sheet points one of them at another
...
moa_keys <- moa |> transmute(partnership_id, moa_file, file_key = str_to_lower(moa_file))
dis_moa <- moa_keys |>
  inner_join(moa_keys |> select(file_key, other = partnership_id), by = "file_key", relationship = "many-to-many") |>
  filter(other != partnership_id) |>
  distinct(partnership_id, moa_file, other) |>
  transmute(partnership_id, kind = "moa_file", field = "moa_files", value_a = moa_file, source_a = "moa_link",
            value_b = other, source_b = "moa_link", published_value = moa_file,
            resolution_rule = "the same MOA file is linked for another partnership; recorded, not reassigned")
disagreements <- bind_rows(dis_state, dis_date, dis_model, dis_presence, dis_ambiguous, dis_moa) |>
  select(kind, partnership_id, field, value_a, source_a, value_b, source_b, published_value, resolution_rule) |>
  arrange(kind, partnership_id, source_a)

stopifnot(
  "every partnership id is unique" = !anyDuplicated(partnerships$partnership_id),
  "every ICE partnership is present" = all(ice$partnership_id %in% partnerships$partnership_id),
  "every resolved sheet row belongs to a partnership" =
    !any(is.na(obs_ids$partnership_id) & !is.na(obs_ids$agreement_id))
)

dir.create("data", showWarnings = FALSE)
arrow::write_parquet(partnerships, "data/partnerships.parquet")
...
unresolved <- claims |>
  ...
  distinct(source_id, state, agency_raw, resolution, partnership_id) |>
...
write_csv(disagreements, "data/intermediate/partnership-disagreements.csv", na = "")
...
per_source <- claims |>
  summarise(as_of = if (all(is.na(as_of))) "—" else paste(sort(unique(as.character(as_of))), collapse = ", "),
            partnerships = n_distinct(partnership_id[!is.na(partnership_id)]),
            listed = n_distinct(partnership_id[field %in% c("listed", "pending") & !is.na(partnership_id)]),
            signed = n_distinct(partnership_id[field == "signed" & !is.na(partnership_id)]),
...
summary_md <- c(
  ...
  sprintf("Partnerships: **%d** across every era (%d published by ICE, %d added from ICE press releases); %d current.",
          nrow(partnerships), sum(partnerships$ice_published), sum(!partnerships$ice_published), sum(partnerships$is_current)),
  ...
  "|---|---|---|---|---|---|",
  per_source |> mutate(row = sprintf("| %s | %s | %d | %d | %d | %d |", source_id, as_of, partnerships, listed, signed, unresolved)) |> pull(row)
)
dir.create("data/qa", showWarnings = FALSE)
writeLines(summary_md, "data/qa/historical-summary.md")

message(sprintf("partnerships: %d (%d ICE-published, %d from press releases; %d current)",
                nrow(partnerships), sum(partnerships$ice_published), sum(!partnerships$ice_published), sum(partnerships$is_current)))
