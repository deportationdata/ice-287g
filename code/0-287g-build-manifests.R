...
# converting legacy download_path_log.csv or reconstructing from the files.
...
convert_legacy <- function(folder) {
  legacy <- file.path(folder, "download_path_log.csv")
  log <- read_csv(legacy, col_types = cols(.default = "c"))
...
reconstruct <- function(folder) {
  ...
  if (str_detect(base, "^agreements_\\d{8}_\\d{6}$")) {
    ...
    rows$state <- map_chr(parts, \(p) if (length(p) >= 3) p[1] else NA_character_)
  ...
  } else if (base %in% c("sheets_wayback_20260814", "agreements_wayback_20260814")) {
    rows$retrieved_at <- "2026-08-14"
    rows$note <- "Wayback Machine backfill ingested 2026-08-14; per-file capture urls were not recorded"
    ...
    rows$note <- case_when(
      !is.na(raw_ts) ~ "raw Wayback capture fetched by code/0-287g-wayback-backfill.R",
      str_detect(basename(files), "^html_") ~
        "table extracted from the same-dated raw/ capture by code/0-287g-wayback-backfill.R",
      basename(files) == "cdx_stamps.txt" ~
        "capture worklist cache for code/0-287g-wayback-backfill.R",
    ...
    rows$note <- case_when(
      !is.na(raw_ts) ~
        "pre-2018 fact-sheet capture; see inputs/pre2018/SOURCES.csv",
      str_detect(basename(files), "^html_") ~
        "table extracted from the same-dated raw/ capture by code/0-287g-pre2018-rosters.R",
      TRUE ~ NA_character_
    )
  } else if (base == "agreements_wayback_pre2018") {
    rows$retrieved_at <- "2026-09-12"
    cdx_path <- file.path(folder, "_cdx_index.tsv")
    if (file.exists(cdx_path)) {
      # _cdx_index.tsv is space-separated despite the name
      m <- str_match(read_lines(cdx_path), "^(\\S+)\\s+(\\d{14})$")
      cdx <- tibble(original = m[, 2], timestamp = m[, 3]) |>
        filter(!is.na(original))
      cdx <- cdx |>
        mutate(pdf = basename(original)) |>
        distinct(pdf, .keep_all = TRUE)
      idx <- match(basename(files), cdx$pdf)
      rows$url <- if_else(
        !is.na(idx),
        sprintf("https://web.archive.org/web/%sid_/%s",
                cdx$timestamp[idx], cdx$original[idx]),
        NA_character_
      )
    }
    rows$note <- if_else(
      basename(files) == "_cdx_index.tsv",
      "CDX enumeration cache used by code/0-287g-pre2018-download.R",
      "pre-2018 signed MOA; see inputs/pre2018/SOURCES.csv"
    )
  }
...
for (folder in snapshot_folders) {
  has_legacy <- file.exists(file.path(folder, "download_path_log.csv"))
  has_manifest <- file.exists(file.path(folder, "manifest.csv"))
  if (has_legacy) {
    convert_legacy(folder)
