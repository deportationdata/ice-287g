...
agreement_identifiers <- arrow::read_parquet("data/intermediate/agreements.parquet") |>
  ...
  mutate(
    ...
    crime_rank = match_tier_rank(crime_match_type) + 1.5 * (crime_ori_ambiguous & crime_match_type == "exact_state_county_agency_name"),
    lear_rank = match_tier_rank(lear_match_type) + 1.5 * (lear_ori_ambiguous & lear_match_type == "exact_state_county_agency_name"),
    leaic_rank = match_tier_rank(leaic_match_type) + 1.5 * (leaic_ori_ambiguous & leaic_match_type == "exact_state_county_agency_name"),
