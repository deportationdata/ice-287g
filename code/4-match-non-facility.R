...
non_facility_layers <- list(
  ...
  municipal = st_read("data/intermediate/match-municipal.parquet", quiet = TRUE),
  pa_constable = st_read("data/intermediate/match-pa-constable.parquet", quiet = TRUE)
)
...
non_facility_layers$precinct <- agreements |>
  filter(geom_class == "precinct_polygon") |>
...
# a placed polygon always carries the census code the slicer keys on
stopifnot(
  "every placed non-facility feature carries a geoid" =
