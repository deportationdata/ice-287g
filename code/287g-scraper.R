...
# log what was found
cat(sprintf("Found %d participating link(s): %s\n", length(participating), paste(participating, collapse = ", ")))
cat(sprintf("Found %d pending link(s): %s\n",       length(pending),       paste(pending, collapse = ", ")))
...
if (length(participating) == 0) {
  ...
  for (link in all_links) {
    cat(sprintf("  [%s] %s\n", html_text(link, trim = TRUE), html_attr(link, "href")))
  }
...
download_excel_from_url <- function(url, folder, label = "file") {
  tryCatch({
    ...
    if (!is.null(cd) && grepl("filename=", cd)) {
      file_name_only <- str_trim(str_remove(str_split(cd, "filename=")[[1]][2], '["\']'))
    } else {
      # fall back to last segment of URL
      file_name_only <- basename(str_split(url, "\\?")[[1]][1])
      if (is.na(file_name_only) || file_name_only == "" || !grepl("\\.", file_name_only)) {
...
# create a timestamped subfolder
timestamp_folder <- file.path(documents_folder, paste0("agreements_", timestamp))
...
for (i in seq_along(hyperlinks_list)) {
  ...
  tryCatch({
    ...
    if (status_code(r) == 200) {
      ...
      cd <- headers(r)[["content-disposition"]]
      if (!is.null(cd) && grepl("filename=", cd)) {
        fname <- str_trim(str_remove(str_split(cd, "filename=")[[1]][2], '["\']'))
      } else {
...
if (length(failed) > 0) {
  ...
  cat(sprintf("%d failed downloads logged to %s\n", length(failed), failed_log_path))
}
