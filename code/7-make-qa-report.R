...
level_sf <- st_read("data/agreements-sf.parquet", quiet = TRUE)
ids <- arrow::read_parquet("data/intermediate/match-agency-identifiers.parquet")
missing <- arrow::read_parquet(
  "data/intermediate/match-missing-identifiers.parquet"
)
identities <- arrow::read_parquet(
  "data/intermediate/identity-agreements.parquet"
)
...
manifests <- list.files(
  ...
  map(\(p) {
    read_csv(p, col_types = cols(.default = "c"), progress = FALSE) |>
...
summary <- bind_rows(
  ...
  check(
    "identities folding more than one spelling",
    sum(identities$n_spellings > 1)
  ),
  identities |>
    count(identity_resolution, name = "n") |>
    pmap(\(identity_resolution, n) {
      check("identities by resolution", n, scope = identity_resolution)
    }) |>
    list_rbind(),
  check(
  ...
  check(
    ...
    sum(diff(pubs$published_on[order(pubs$pub_seq)]) < 0),
    0
  ),
  pubs |>
    filter(!is.na(date_flag)) |>
    count(date_flag, name = "n") |>
    pmap(\(date_flag, n) {
      check("sheet publications with a date flag", n, scope = date_flag)
    }) |>
    list_rbind(),
  check(
  ...
  check(
    "duplicate identity rows in agreements",
    sum(duplicated(agreements[c("state", "agency", "support_type", "signed")])),
    0
  ),
  check("distinct states in agreements", n_distinct(agreements$state)),
  check("feature rows", nrow(all_sf)),
  check(
    "agreement-level rows == agreements rows",
    nrow(level_sf),
    nrow(agreements)
  ),
  check(
    "agreements with no feature row",
    sum(!agreements$agreement_id %in% all_sf$agreement_id),
    0
  ),
  features |>
    count(match_layer, name = "n") |>
    pmap(\(match_layer, n) {
      check("feature rows by layer", n, scope = match_layer)
    }) |>
    list_rbind(),
  features |>
    filter(empty) |>
    count(match_layer, name = "n") |>
    pmap(\(match_layer, n) {
      check("empty-geometry features by layer", n, scope = match_layer)
    }) |>
    list_rbind(),
  check(
    "active features flagged needs_review",
    sum(active_features$needs_review)
  ),
  active_features |>
    count(match_quality, name = "n") |>
    pmap(\(match_quality, n) {
      check("active features by match quality", n, scope = match_quality)
    }) |>
    list_rbind(),
  check(
  ...
  check(
    "county geoids outside the census county list",
    sum(
      !is.na(features$geoid[features$match_layer == "county"]) &
        !features$geoid[features$match_layer == "county"] %in% census_counties
    )
  ),
  check(
    "DOC agreements fanned out to state prisons",
    n_distinct(features$agreement_id[str_starts(
      coalesce(features$match_type, ""),
      "doc_"
    )])
  ),
  check(
    "agreement-identifiers rows == agreements rows",
    nrow(ids),
    nrow(agreements)
  ),
  check(
    "active agreements with an ORI9",
    sum(
      !is.na(ids$ORI9[
        ids$agreement_id %in%
  ...
  ids |>
    count(ori_source, name = "n") |>
    pmap(\(ori_source, n) {
      check("ORI9 by source", n, scope = coalesce(ori_source, "none"))
    }) |>
    list_rbind(),
  ...
  features |>
    filter(!empty) |>
    count(geometry_vintage, name = "n") |>
    pmap(\(geometry_vintage, n) {
      check(
        "placed features by geometry vintage",
        n,
        scope = as.character(geometry_vintage)
      )
    }) |>
    list_rbind(),
  missing |>
    count(missing_identifier_type, name = "n") |>
    pmap(\(missing_identifier_type, n) {
      check("missing identifiers by type", n, scope = missing_identifier_type)
    }) |>
    list_rbind(),
  check(
    "distinct MOA urls on the sheet",
    n_distinct(agreements$moa[str_detect(
      coalesce(agreements$moa, ""),
      "^https?://"
    )])
  ),
  check(
    "distinct MOA urls in agreements manifests",
    n_distinct(manifests$url[str_detect(
      coalesce(manifests$url, ""),
      "^https?://"
    )])
  ),
  check(
    "agreements folders with outstanding failed downloads",
    length(Filter(
      \(p) file.size(p) > 0,
      list.files(
        "agreements",
  ...
  check(
    "MOA urls the sheet links that the origin reports gone and no archive holds",
    list.files(
      "agreements",
      "^unreachable\\.csv$",
      recursive = TRUE,
      ...
      (\(d) {
        if (nrow(d)) {
          sum(
            !unique(d$url) %in% manifests$url[file.exists(manifests$saved_path)]
          )
        } else {
          0L
        }
...
map(folders, \(d) {
  ...
  disk <- setdiff(
    ...
    c("manifest.csv", "download_path_log.csv", "failed_downloads.txt")
  )
  urls <- m |>
  ...
  tibble(
    ...
    files_on_disk = length(disk),
    ghost_rows = sum(!exists_anywhere),
    urls_with_multiple_hashes = sum(urls$n > 1),
    failed_download_lines = if (file.exists(failed)) {
      length(readLines(failed, warn = FALSE))
    } else {
      0L
    }
  )
...
cat(sprintf(
  "qa: %d checks, %d pass, %d fail, %d info\n",
  nrow(summary),
  sum(summary$status == "pass"),
  nrow(fails),
  sum(summary$status == "info")
))
if (nrow(fails)) {
  print(as.data.frame(fails |> select(check, scope, value, expected)))
}
if (strict && nrow(fails)) {
  stop("QA_STRICT: ", nrow(fails), " failing check(s)")
}
