...
The partnership history is built from ICE's own records plus one DHS OIG
appendix. Every file here is read by `code/1-read-historical-*.R` or
`code/2-make-partnerships.R`; the secondary sources the pipeline does not read
...
| `ice_archive_index` | ICE's live `/287g-archive` index of MOA documents with original signing dates, retrieved 20 Apr 2026. It also files non-287(g) documents (a Cook County forfeiture MOU, a Morristown application letter), so it may not add a partnership | `ice_live_287gMOA_index_2026-04-20.tsv` |
...
only `ice_press` may add a partnership. `SOURCES.csv` is the citation
...
- `2-make-partnerships.R` first reduces each source to typed claims (`listed`,
  `pending`, `signed`, `model`, `moa_file`, `rescinded`) and resolves each to a
  partnership by rule — the registry of ICE's spellings and the alias table,
  then a name that begins exactly one partnership, then a signing date unique
...
- It then writes one row per partnership: ICE's listing window
  ...
  `data/intermediate/partnership-disagreements.csv`. There are no stated-total checks,
  source counts or confidence grades: a partnership is on a list or it is not.
...
- ICE's own count of every agreement ever signed through mid-2009 — OIG-10-63
  ...
  terminated (Barnstable) — matches the 67 partnerships on ICE's captures by
